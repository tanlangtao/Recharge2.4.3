...
export default class payCash extends cc.Component {
    ...
    sxAmount = 0 //受限金额
    onLoad () {
        this.app = cc.find('Canvas/Main').getComponent('payMain_22');
        this.fetchIndex();
        //设置延迟，避免用户频繁操作导致报错
            ...
            if(response.status == 0){
                ...
                ids.forEach(e=>{
                    if(e == this.app.UrlData.user_id) {
                        ...
                        node.getComponent('payDhToggle_22').init({
                            text:"赠送"
    ...
    public addNavToggle(){
        ...
        //首次加载，顺序第一的显示
        if(arr[0]=='人工兑换'){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('RgDh')
        }else if(arr[0] == "银行卡兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('BankDh')
        }else if(arr[0] == "支付宝兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('Dh')
        }else if(arr[0] == "USDT兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('UsdtDh')
        }else if(arr[0] == "兑换记录"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('DhHistory')
        }else if(arr[0] == "极速兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('JisuDh')
        }else if(arr[0] == "匹配兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('PipeiDh')
        }else if(arr[0] == "极速兑换Iframe"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_22').addContent('JisuDhIframe')
        }
    }
    onDestroy(){
        clearTimeout(this.timer)
