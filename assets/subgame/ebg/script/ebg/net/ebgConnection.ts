...
export default class Connection {
  ...
  public Create(serverurl: string) {
    ...
    if(cc.sys.platform === cc.sys.ANDROID || cc.sys.os === cc.sys.OS_ANDROID){ //在浏览器调试中，cc.sys.os === cc.sys.OS_ANDROID 是true
      if(serverurl.indexOf('wss') == -1){
          this.ws = new WebSocket(serverurl);
      }else{
          this.ws = new WebSocket(serverurl, cc.url.raw('resources/hall/cacert.pem'));
      }
    }else{
        this.ws = new WebSocket(serverurl);
    }
  ...
  public onDispatchMessage(id: number, data: any) {
    cc.log('onDispatchMessage!this function need overload~!')
  }
  ...
  public SendMessage(tag: number, buff: any) {
    ...
    let callback = function() {
        if (this.ws.readyState === WebSocket.OPEN) {
            ...
            let tip = Language_ebg.Lg.changeLanguage(13)
            gHandler.eventMgr.dispatch(gHandler.eventMgr.showTip,tip)
            gHandler.eventMgr.dispatch(gHandler.eventMgr.showTip,tip)
        }
    }
