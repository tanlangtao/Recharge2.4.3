...
import Config from "./payConfig_25"
...
export default class NewClass extends cc.Component {
    ...
    public showAlert(text:string){
        ...
        node.getComponent('payPublicAlert_25').init(text)
    }
    /**
     * 自制键盘输入的类型
     * @param e 内容
    ...
    public showOrderAlert(type,data,beforePay = true){
        ...
        node.getComponent('payPublicOrderAlert_25').init(type,data)
    }
    /**
     * 添加银行卡类型弹窗
     * @param data 
    ...
    public showBankAccountAlert(data){
        ...
        let BankAccountAlert = node.getComponent('payBankAccountAlert_25');
    ...
    public showUsdtAccountAlert(itemId,type){
        ...
        let payUsdtAccountAlert = node.getComponent('payUsdtAccountAlert_25');
    ...
    public showBankTipAlert(component){
        ...
        node.getComponent('payBankTipAlert_25').init(component)
        //检测是否已存在弹窗，避免重复显示
    ...
    public showKeyBoard(label,type,callBack = ()=>{}){
        ...
        canvas.addChild(node);
        node.getComponent('payKeyBoardAlert_25').init(label,type,callBack)
    }
    /**
     * 设置输入框字体颜色
     * @param msg 
    ...
    setInputColor(msg,input){
        let color1 = new cc.Color(133, 147, 186);
        let color2 = new cc.Color(133, 147, 186);
        //设置字的颜色
    ...
    showZfbWxAlert(url){
        ...
        node.getComponent('payZfbWxAlert_25').init(url)
    }
