...
export default class NewClass extends cc.Component {
    ...
    infoInit(){
        ...
        let group2 = cc.find("Canvas/Activity/Content/Bwfcjl/bg/group2")
        let group3 = cc.find("Canvas/Activity/Content/Bwfcjl/bg/group3")
    ...
    getReceivePerformanceInfo(){
        var url = `${this.app.UrlData.host}/api/activity/getReceivePerformanceInfo?user_id=${this.app.UrlData.user_id}&activity_id=${this.activity_id}&source=2`;
        let self = this;
    ...
    receivePerformanceGold(){
        ...
        let self = this;
        let dataStr = `user_id=${this.app.UrlData.user_id}&index=${this.targeIndex}&source=2&package_id=${this.app.UrlData.package_id}&activity_id=${this.activity_id}&login_ip=${this.login_ip ? this.login_ip:this.app.gHandler.gameGlobal.regin_ip}&regin_ip=${this.app.gHandler.gameGlobal.regin_ip}&device_id=${this.app.gHandler.app.deviceID}`
    ...
    setLanguageResource(){
        ...
        let bg= cc.find('Canvas/Activity/Content/Bwfcjl/bg')
        this.app.loadIconLg(`${src}/activeBigImage/XG_event_20210625-5`,bg)
        let rule = cc.find("Canvas/Activity/Content/Bwfcjl/bg/rule").getComponent(cc.Label)
        rule.string=`1. 玩家需要绑定手机号和银行卡才能参与。\n2. 统计规则：只统计棋牌类游戏+第三方电子类游戏的有效投注。\n3. 玩家上周团队总业绩和我的总返佣均达标可前往活动界面进行领取彩金。\n4. 领取时间： 每周一${this.app.config.transitionTime(this.info.start)}-${this.app.config.transitionTime(this.info.end)}， 逾期未领取视为自动放弃。\n5. 同一用户（同IP同设备视为同一用户）仅限参加一次活动，活动彩金可直接申请兑换。\n6. 平台拥有最终解释权，严禁一切恶意行为，出现违规情况，一律封号处理；同时平台有权根据实际情况，随时调整活动内容。`
    }   
}
