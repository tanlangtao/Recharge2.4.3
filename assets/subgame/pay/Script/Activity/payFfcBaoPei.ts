...
export default class NewClass extends cc.Component {
    ...
    setLanguageResource(){
        ...
        rule.string = `${Language_pay.Lg.ChangeByText("<color=#FFFFFF>1. 新玩家注册好账号，需先绑定好手机号码与银行卡后联系上级进行申请，申请时间：每天12:00--18:00。\n2. 平台中的新玩家活动只能参加其中一个。</c><color=#F3DC5B>\n3. 活动限制：仅限彩源猜大小-河内分分彩房间，单局下注仅限一个区域（大或小），不能投注豹子，进行其他游戏视为放弃此活动。</c>\n4. 在规定游戏中连续下注")}${this.info.round}${Language_pay.Lg.ChangeByText("局且单局下注金额为")}${this.info.bet_min}-${this.info.bet_max}${Language_pay.Lg.ChangeByText("金币，依照累计产生的净亏损前往本活动界面进行领取活动彩金。\n5. 同一用户（同IP同设备视为同一用户）仅限参加一次活动，活动彩金需")}${this.info.flow_rate}${Language_pay.Lg.ChangeByText("倍流水方可申请兑换。\n6. 平台拥有最终解释权，严禁一切恶意行为，出现违规情况，一律封号处理；同时平台有权根据实际情况，随时调整活动内容。</color>")}`
    }   
}
