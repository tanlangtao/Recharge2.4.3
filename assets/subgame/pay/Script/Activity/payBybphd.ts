...
export default class NewClass extends cc.Component {
    ...
    infoInit(){
        let group1 = cc.find("Canvas/Activity/Content/Bybphd/bg/event4_xzcyh_frame/group/group1")
        let group2 = cc.find("Canvas/Activity/Content/Bybphd/bg/event4_xzcyh_frame/group/group2")
        let group3 = cc.find("Canvas/Activity/Content/Bybphd/bg/event4_xzcyh_frame/group/group3")
        this.info.withdraw_conf.condition.forEach((e,index)=>{
            group1.children[index].getComponent(cc.Label).string = `${e.recharge_amount}`
            group2.children[index].getComponent(cc.Label).string = `${this.info.conf[index].gold}`
            group3.children[index].getComponent(cc.Label).string = `${e.max_withdraw_amount}`
        })
    }
    getFristPayAmount(){
    ...
    ApplyBtnInit(){
        let btn= cc.find('Canvas/Activity/Content/Bybphd/bg/event4_xzcyh_frame/applyBtn').getComponent(cc.Button)
    ...
    getApplyLocal(){
        let local = cc.sys.localStorage.getItem(`ApplyBybphd_${this.app.UrlData.user_id}`)
        if(local){
            return false
        }else{
            return true
    ...
    setApplyLocal(){
        cc.sys.localStorage.setItem(`ApplyBybphd_${this.app.UrlData.user_id}`,JSON.stringify(true))
    }
    setLocal(){
