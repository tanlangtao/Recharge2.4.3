...
export default class NewClass extends cc.Component {
    ...
    infoInit(){
        let label2 = cc.find("Canvas/Activity/Content/CdxQiQu/bg/Layout/label2").getComponent(cc.Label)
        let label3 = cc.find("Canvas/Activity/Content/CdxQiQu/bg/Layout/label3").getComponent(cc.Label)
        let group1 = cc.find("Canvas/Activity/Content/CdxQiQu/bg/Layout/group1")
        let group2 = cc.find("Canvas/Activity/Content/CdxQiQu/bg/Layout/group2")
    ...
    //     })
    // }
    getGameHandleQiQuWin(){
        let gameHost = this.app.UrlData.host.replace("pay","game")
        var url = `${gameHost}/caidaxiao/api/HandleQiQuWin?user_id=${this.app.UrlData.user_id}&level_amount=${this.info.win}`;
        let self = this;
        this.app.ajax('GET',url,'',(response)=>{
            if(response.code == 0){
    ...
    receiveHandleHeNeiWin(){
        ...
        let self = this;
        let dataStr = `user_id=${this.app.UrlData.user_id}&user_name=${this.app.UrlData.user_name}&flag=2&package_id=${this.app.UrlData.package_id}&activity_id=${this.activity_id}&login_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&regin_ip=${this.app.gHandler.gameGlobal.regin_ip}&device_id=${this.app.gHandler.app.deviceID}`
        // let dataStr = `user_id=${this.app.UrlData.user_id}&flag=1&package_id=${this.app.UrlData.package_id}&activity_id=${this.activity_id}&login_ip=127.0.0.1&regin_ip=127.0.0.1&device_id=123456789`
    ...
    setLanguageResource(){
        ...
        let cdxbg_qq= cc.find('Canvas/Activity/Content/CdxQiQu/bg/cdxbg_qq')
        this.app.loadIconLg(`${src}/activeBigImage/cdxbg_qq`,cdxbg_qq)
        this.btnArr.forEach(e=>{
