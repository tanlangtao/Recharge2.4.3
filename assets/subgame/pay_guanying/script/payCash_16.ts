...
export default class NewClass extends cc.Component {
    ...
    onLoad () {
        ...
        if(this.app.UrlData.package_id != 16 && this.app.UrlData.package_id != 35){
            if(scalex >1.1){
        ...
        }else if(this.app.UrlData.package_id == 16 || this.app.UrlData.package_id == 35 ){
            //渠道16才显示受限金额 
        }
            ...
            if(response.status == 0){
                ...
                ids.forEach(e=>{
                    if(e == this.app.UrlData.user_id) {
                        ...
                        node.getComponent('payDhToggle_16').init({
                            text:"赠送"
    ...
    public addNavToggle(){
        ...
        if(this.results.data.withDraw_info.artificial){
            if(this.results.data.withDraw_info.artificial.is_close > 0){
                //15不开放人工兑换
                if(this.app.UrlData.package_id == 15|| this.app.UrlData.package_id == 16 || this.app.UrlData.package_id == 35 || this.app.UrlData.package_id == 20){
                    return
                }
                arr.push('人工兑换')
            }
        ...
        //首次加载，顺序第一的显示
        if(arr[0]=='人工兑换'){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('RgDh')
        }else if(arr[0] == "银行卡兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('BankDh')
        }else if(arr[0] == "支付宝兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('Dh')
        }else if(arr[0] == "USDT兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('USDT')
        }else if(arr[0] == "兑换记录"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('DhHistory')
        }else if(arr[0] == "极速兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('极速兑换')
        }else if(arr[0] == "匹配兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('匹配兑换')
        }else if(arr[0] == "极速兑换Iframe"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('JisuDhIframe')
        }else if(arr[0] == "代付支付宝兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('AlipayDh')
        }else if(arr[0] == "代付银行卡兑换"){
            this.ToggleContainer.children[0].getComponent('payDhToggle_16').addContent('BankDhDaiFu')
        }
    }
    onDestroy(){
        clearTimeout(this.timer)
