...
export default class NewClass extends cc.Component {
    ...
    public initRender(){
        ...
        this.bankInfo.getChildByName("bankName").getComponent(cc.Label).string = this.Info.account_name
        if(this.Info.account_name == '' ||this.Info.account_card =='' ){
    ...
    //显示弹窗
    showAccountAlert(){
        this.node.getChildByName("bindAlipayAccount").active = true
        this.node.getChildByName("Bank").active = false
        this.node.getComponent("payAlipayAccountAlert_16").init({
            text:'设置支付宝',
            action:this.action,
            itemId:this.bankId,
    ...
    onClick(){
        ...
        if(this.results.length==0){
            ...
            this.app.showBankTipAlert("请完善支付宝绑定信息",this.showAccountAlert.bind(this))
