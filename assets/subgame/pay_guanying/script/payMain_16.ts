...
export default class NewClass extends cc.Component {
    ...
    public showAlert(text:string){
        ...
        node.getComponent('payPublicAlert_16').init(text)
    }
    /**
     * 自制键盘输入的类型
     * @param e 内容
    ...
    public showOrderAlert(type,data,beforePay = true){
        ...
        node.getComponent('payPublicOrderAlert_16').init(type,data)
    }
    /**
     * 添加银行卡类型弹窗
     * @param data 
    ...
    public showBankAccountAlert(data){
        ...
        let BankAccountAlert = node.getComponent('payBankAccountAlert_16');
    ...
    public showUsdtAccountAlert(itemId,type){
        ...
        let payUsdtAccountAlert = node.getComponent('payUsdtAccountAlert_16');
     ...
     * @param data 
     */
    public showBankTipAlert(text,callback){
        var canvas = cc.find('Canvas');
        var node = null;
        ...
        if(!cc.find("Canvas/BankTipAlert")){
            canvas.addChild(node);
            node.getComponent('payBankTipAlert_16').init(text,callback)
        }
    }
    /**
     * 小键盘 
    ...
    public showKeyBoard(label,type,callBack = ()=>{}){
        ...
        canvas.addChild(node);
        node.getComponent('payKeyBoardAlert_16').init(label,type,callBack)
    }
    /**
     * 设置输入框字体颜色
     * @param msg 
    ...
    setInputColor(msg,input){
        ...
        }else if(this.UrlData.package_id == 16 || this.UrlData.package_id == 35 ){
    ...
    showZfbWxAlert(url){
        ...
        node.getComponent('payZfbWxAlert_16').init(url)
    }
