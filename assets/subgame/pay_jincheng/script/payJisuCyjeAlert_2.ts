...
export default class NewClass extends cc.Component {
    ...
    onLoad(){
        this.cash = cc.find('Canvas/Cash').getComponent('payCash_2')
    ...
    renderBtn(list){
        list.forEach((e) => {
            ...
            node.getComponent("payBtnNum_2").init(e,this.callBack.bind(this))
    ...
    renderBtn2(list){
        list.forEach((e) => {
            var node = cc.instantiate(this.btnNum)
            node.getComponent("payBtnNum_2").init(e.amount,this.callBack.bind(this),e.id)
            this.content.addChild(node)
        });
    }
    callBack(e,id){
