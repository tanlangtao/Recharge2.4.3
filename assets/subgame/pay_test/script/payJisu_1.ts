...
export default class NewClass extends cc.Component {
        ...
        this.app.ajax('GET',url,'',(response)=>{
            if(response.status == 0){
                ...
                response.data.amount_list.forEach((e)=>{
                    ...
                    node.getComponent("payBtnNum_1").init(e.amount,this.addGold.bind(this),e.id)
    ...
    public showJisuOrderAlert(type,data,beforePay = true){
        ...
        node.getComponent('payJisuOrderAlert_1').init(type,data,()=>{},this.channel)
    }
    radioList(){
            ...
            if(show){
                if(this.results[i].rate != "" && this.results[i].rate !="0.0000"){
                    ...
                    if(packageArr.indexOf(this.app.UrlData.package_id)>-1){
                        ...
                        node.getComponent('paySelectItem_1').init({
    ...
    showBindBankAccountTip(){
        ...
        node.getComponent("payBindBankAccountTipAlert_1").init(this)
        //检测是否已存在弹窗，避免重复显示
    ...
    showAccountAlert_16(){
        ...
        this.node.getComponent("payBankAccountAlert_1").init({
            text:'设置银行卡',
            action:"add",
            itemId:null,
            parentComponent:this,
