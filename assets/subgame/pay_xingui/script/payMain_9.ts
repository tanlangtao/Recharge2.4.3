...
import Config from "./payConfig_9"
...
export default class NewClass extends cc.Component {
    ...
    public showAlert(text:string){
        ...
        node.getComponent('payPublicAlert_9').init(text)
    }
    /**
     * 自制键盘输入的类型
     * @param e 内容
    ...
    public showOrderAlert(type,data,beforePay = true){
        ...
        node.getComponent('payPublicOrderAlert_9').init(type,data)
    }
    /**
     * 添加银行卡类型弹窗
     * @param data 
    ...
    public showBankAccountAlert(data){
        ...
        let BankAccountAlert = node.getComponent('payBankAccountAlert_9');
    ...
    public showUsdtAccountAlert(itemId,type){
        ...
        let payUsdtAccountAlert = node.getComponent('payUsdtAccountAlert_9');
    ...
    public showBankTipAlert(component){
        ...
        node.getComponent('payBankTipAlert_9').init(component)
        //检测是否已存在弹窗，避免重复显示
    ...
    public showKeyBoard(label,type,callBack = ()=>{}){
        ...
        canvas.addChild(node);
        node.getComponent('payKeyBoardAlert_9').init(label,type,callBack)
    }
    /**
     * 设置输入框字体颜色
     * @param msg 
    ...
    showZfbWxAlert(url){
        ...
        node.getComponent('payZfbWxAlert_9').init(url)
    }
