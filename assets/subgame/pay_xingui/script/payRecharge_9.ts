...
export default class NewClass extends cc.Component {
    ...
    public addNavToggle() {
        ...
        for (let i: number = 0; i < arr.length; i++) {
            ...
            node.getComponent('payNavToggle_9').init({
                text: arr[i]
            },this.zfbResults.data.discount_rate)
        }
        //首次加载，顺序第一的显示
        if(arr[0]=='人工代充值'){
            node.getComponent('payNavToggle_9').addDc()
        }else if(arr[0]=='支付宝' && this.zfbResults.data.alipay.length > 0  ){
            node.getComponent('payNavToggle_9').addContent('alipay')

        }else if(arr[0]=='转账到银行卡' &&  this.zfbResults.data.bankcard_transfer.length > 0 ){
            node.getComponent('payNavToggle_9').addContent('bankcard_transfer')

        }else if(arr[0]=='银联扫码' && this.zfbResults.data.union_pay.length > 0 ){
            node.getComponent('payNavToggle_9').addContent('union_pay')

        }else if(arr[0]=='微信' && this.zfbResults.data.wechat_pay.length > 0 ){
            node.getComponent('payNavToggle_9').addContent('wechat_pay')

        }else if(arr[0]=='快捷支付' && this.zfbResults.data.quick_pay.length > 0){
            node.getComponent('payNavToggle_9').addContent('quick_pay')

        }else if(arr[0]=='网银支付' && this.zfbResults.data.bank_pay.length > 0  ){
            node.getComponent('payNavToggle_9').addContent('bank_pay')
        }else if(arr[0]=='IM充值' && this.zfbResults.data.im_pay.length > 0  ){
            node.getComponent('payNavToggle_9').addContent('im_pay')
        }else if(arr[0]=='USDT' && this.zfbResults.data.digiccy.length > 0  ){
            node.getComponent('payNavToggle_9').addContent('digiccy')
